...
function init_wc_paynet_payment_gateway(): void
{
    ...
    if (!class_exists('WC_Payment_Gateway') || class_exists('WC_Payneteasy')) return;

    /**
     * Payment system PAYNETEASY class.
     */
    class WC_Payneteasy extends WC_Payment_Gateway
    {
        // Payment system PAYNETEASY settings
        ...
        /**
         * Конструктор для класса плагина "Платежная система PAYNETEASY".
         */
        ...
        /**
         * Инициализируйте поля формы настроейк платежной системы PAYNET.
         *
         * @return void
         */
...
//        }
        ...
        private function get_pay_url()
        {
            ...
            if (get_option('permalink_structure') == '') {
                $wc_api_return_url = get_site_url() . sprintf('?wc-api=%s_return', $this->id) . '&orderId=' . $order_id;
            } else {
                $wc_api_return_url = get_site_url() . sprintf('/wc-api/%s_return', $this->id) . '&orderId=' . $order_id;
            }
...
//                'redirect_success_url' => $this->get_return_url($this->order),
//                'redirect_fail_url' => wc_get_cart_url(),
                'redirect_success_url' => $wc_api_return_url,
                'redirect_fail_url' => $wc_api_return_url,
